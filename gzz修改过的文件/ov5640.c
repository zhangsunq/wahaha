	...
	#ifndef CONFIG_TC4_EVT     //gzz change
	...
	//#define OV5640_I2C_NAME  "ov5640"  //gzz for mipi change
	#define OV5640_I2C_NAME  "ov5640_mipi"
	...
	#define QSXGA_HEIGHT	1944  //normally 2048, but ov5640 only support simple qsxga
	...
	#if 1
	//input 24M 
	//output 720p 60fps bit rate 672M bps
	{0x3035, 0x11}, // pll
	{0x3036, 0x54}, // pll
	...
	{0x5001, 0x83}, // SDE on, scale off, UV average off, CMX on, AWB on
        ...
        #if 0
       //input 24M 
//output 5M 15fps bit rate 672M bps
{0x3035, 0x11}, // pll
{0x3036, 0x54}, // pll
{0x3c07, 0x07}, // light meter 1 threshold
...
{0x5001, 0x83}, // SDE on, scale off, UV average off, CMX on, AWB on
	...
	*/
	...
	static const struct regval ov5640_qsxga_to_uxga_regs[] = {
		...
		{0x3503, 0x00},//default 0x07  AEC Manual Mode
		{0x3a00, 0x38},
		{0x4050, 0x6e},
		{0x4051, 0x8f},
		{0x5302, 0x1c},
	...
	static const struct regval ov5640_my1080p_regs[] = {
	    //1080P
	    {0x3800, 0x00},
	    {0x3801, 0x10},
	    {0x3802, 0x00},
	    {0x3803, 0x10},
	    {0x3804, 0x7},//max x =2592
	    {0x3805, 0xd0},
	    {0x3806, 0x5},//max y=1944
	    {0x3807, 0xdc},

	    /* the output weight and height*/
	    {0x3808, 0x5 }, /* 0x400==1024 */
	    {0x3809, 0x00},
	    {0x380a, 0x4 }, 
	    {0x380b, 0x38},
	    /* // 720P
	    ...
	    */
	    ...
	    {0x3810, 0x00}, //yuanlai 640*480 de
	    {0x3811, 0x10}, 
	    {0x3812, 0x00}, 
	    {0x3813, 0x06}, 
	...
	static const struct regval ov5640_my720p_regs[] = {
	//input 24M 
	//output 720p 60fps bit rate 672M bps
	{0x3035, 0x11}, // pll
	{0x3036, 0x54}, // pll
	...
	static const struct ov5640_win_size ov5640_wins[] = {
	    ...
	    {
		.name    = "my720P",    /* XGA: 1024*768 */
		.resv    = RESV_XGA,
		.width   = 1280,
		.height  = 720,
		.regs    = ov5640_my720p_regs,
	    },
	    {
		...
		.regs    = ov5640_qsxga_to_uxga_regs,
	     ...
	     {
		.name    = "MY1080p",    /* UXGA: 1600*1200 */
		.resv    = RESV_UXGA,
		.width   = 1920,
		.height  = 1080,
		.regs    = ov5640_my1080p_regs,
	    },
	    {
	...
	static int ov5640_power(int flag)
	{
		...
		regulator_enable(ov_vdd18_cam_regulator);
			...
			err = gpio_request(EXYNOS4212_GPM4(7), "GPM4_7");
			if (err)
				printk(KERN_ERR "#### failed to request GPL0_3 ####\n");
			s3c_gpio_setpull(EXYNOS4212_GPM4(7), S3C_GPIO_PULL_NONE);
			gpio_direction_output(EXYNOS4212_GPM4(7), 0);
			gpio_free(EXYNOS4212_GPM4(7));
			msleep(20);

			//reset
			err = gpio_request(EXYNOS4212_GPM4(6), "GPM4_6");
			if (err)
			...
			msleep(10);
			gpio_direction_output(EXYNOS4212_GPM4(6), 0);
			msleep(10);
			gpio_direction_output(EXYNOS4212_GPM4(6), 1);
	    ...
	    unsigned int width, unsigned int height)
	{
		...
		tmp = abs(width - ov5640_wins[i].width) + 
	      ...
	      printk("gzz select_win tmp is %d,diff is %d\n",tmp,diff);
		if (tmp < diff) {
		    diff = tmp;
		    ret = ov5640_wins + i;
		}
	...
	static int ov5640_s_stream(struct v4l2_subdev *sd, int enable)
	{
		...
		i2cc_set_reg(client, 0x300e, 0x45);  ///GZZ CHANGE
	      i2cc_set_reg(client, 0x3008, 0x02); /// GZZ CHANGE
	     i2cc_set_reg(client, 0x4202, 0x00); 
	    ...
	    unsigned int *height, enum v4l2_mbus_pixelcode code)
	{
	    ...
	    printk("gzz -- init ov9640 regs start\n");
	  //  if (RESV_VGA == priv->win->resv)
	   ...
	   printk("gzz -- init ov9640 regs finished\n");
					  ...
					  struct v4l2_frmsizeenum *fsize)
	{
		...
		struct i2c_client *client = v4l2_get_subdevdata(sd);//gzz chamge
		struct ov5640_priv *priv = to_ov5640(client);
		
		//printk("********************** %s(%d) **************************\n", __FUNCTION__, __LINE__);
	        unsigned int width = priv->win->width;
               	
		fsize->type = V4L2_FRMSIZE_TYPE_DISCRETE;
	       switch (width)
               {
        	case 1280:fsize->discrete.width = 1280;//1280;//state->pix.width;
	        fsize->discrete.height = 720;break; //720;//state->pix.height;
                case 1920:fsize->discrete.width = 1920;
                fsize->discrete.height = 1080;break;
                case 2592:fsize->discrete.width = 2592;
                fsize->discrete.height = 1944;break; 
                default : fsize->discrete.width = 640;fsize->discrete.height = 480;break;
               }
	return 0;
}
